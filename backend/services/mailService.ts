...
class MailService {
  ...
  public async notifyAdminSupportTicket(ticket: any) {
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
        <div style="background: #0E5E58; padding: 20px; text-align: center; color: #ffffff;">
          <h2 style="margin: 0; font-size: 20px;">📩 Customer Inquiry / Support Ticket</h2>
        </div>
        <div style="padding: 24px; color: #1f2937;">
          <p>Hello Admin,</p>
          <p>A customer has submitted a new inquiry through Hound & Harbor customer care:</p>
          <div style="background: #f9fafb; padding: 16px; border-radius: 6px; margin: 16px 0;">
            <p style="margin: 4px 0;"><strong>Subject:</strong> ${ticket.subject}</p>
            <p style="margin: 4px 0;"><strong>Category:</strong> ${ticket.category || 'General'}</p>
            ...
            <div style="margin-top: 12px; padding: 12px; background: #ffffff; border-left: 4px solid #0E5E58; border-radius: 4px;">
              <p style="margin: 0; font-style: italic; white-space: pre-wrap;">"${ticket.message || ticket.initialMessage}"</p>
            </div>
          </div>
          <p>You can reply directly to this email or contact the user at <a href="mailto:${ticket.customerEmail}">${ticket.customerEmail}</a>.</p>
        </div>
      </div>
    `;
    ...
    await this.sendMail({
      to: ADMIN_EMAIL,
      subject: `📩 [Hound & Harbor Support] ${ticket.subject} (From: ${ticket.customerEmail})`,
      html,
    });
  }
